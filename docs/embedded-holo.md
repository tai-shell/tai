...
- **Apple Developer ID notarization.** `make -C embedded bundle` appends a gzip tarball + 24-byte trailer *after* the Mach-O segments, which invalidates any prior signature. The release workflow (`.github/workflows/release.yml`) addresses this by running `codesign --force --deep --sign -` AFTER the bundle step — the resulting adhoc signature covers all bytes including the appended payload, and `codesign --verify --deep --strict` passes. CI-built artifacts download and run on stock macOS without requiring `xattr -d com.apple.quarantine`. **Developer-ID-signed and notarized artifacts** (the Mac App Store / wide-distribution flow that requires an Apple Developer certificate) are out of scope; the SSH-then-`scp` distribution model the rest of this design assumes doesn't need them. A future redesign targeting App Store distribution would store the payload as a side-car `.dat` next to the binary (locate via argv[0]-relative path) so the executable's code-signature could be issued separately.

  Note: a *local* `make -C embedded bundle` (a developer testing on their own machine) produces an unsigned single-arch binary. The adhoc-sign step lives only in the CI release workflow; local builds are for development, not distribution.
