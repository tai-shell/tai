...
- **Apple Developer ID notarization + Gatekeeper "no quarantine" pass.** `make -C embedded bundle` appends a gzip tarball + 24-byte trailer *after* the Mach-O segments. Clang's default ad-hoc signature on the arm64 slice (required for arm64 to load at all on Apple Silicon) covers only the Mach-O segments, so the appended payload sits outside the signed region. The binary still loads — same pattern PyInstaller `--onefile` uses — but it isn't notarized, so a freshly-downloaded artifact carries `com.apple.quarantine` and the user has to clear it (`xattr -d com.apple.quarantine /path/to/tai`) before first run. Notarized artifacts (Developer ID + Apple notary service) are out of scope; the SSH-then-`scp` distribution model the rest of this design assumes doesn't need them. A future redesign targeting App Store distribution would store the payload as a side-car `.dat` next to the binary (locate via argv[0]-relative path) so the executable's code-signature could be issued separately.

  Why no post-bundle re-sign: we tried three variants in CI (`codesign --force --deep --sign -`, `codesign --remove-signature` + `codesign --sign -`, and `codesign --force --sign -`) and each either truncated the appended payload or failed codesign's own internal validation. Without a real signing identity the re-sign adds no Gatekeeper benefit on the user side (still need to clear quarantine), so the workflow leaves the existing ad-hoc signature alone.
