...
entire setup transparently:
  ...
  1. Ensures /tmp/ksh on Host B matches our local TAI_BINARY (scp's
     it over only if missing or sha mismatches — typically free
     after first session). The remote install is a single file named
     `ksh`; its basename is a control-shell name, so it serves MCP.
  2. Generates a one-shot launcher.command with a fresh per-session
     ...
     `/tmp/ksh --listen PORT --token T` listener under Terminal.app's
     TCC scope.
...
Env:
    TAI_BINARY     local path to tai-bundled-macos-universal2
                   ...
                   resolved binary's sha256 is compared against /tmp/ksh
...
# The remote install is a single file named `ksh`. The binary switches
# to its MCP-listener (control-shell) behaviour whenever it is invoked
# under a control-shell name, and `ksh` is one — see
# tai_is_control_shell_name in tai_dispatch.c. There is deliberately no
# separate `tai` file or `tcsh` symlink on the remote: one innocuously
# named file, invoked directly as `ksh --listen`.
DEFAULT_REMOTE_BINARY_PATH = "/tmp/ksh"
...
def fire_listener(target: str, port: int) -> str:
    ...
    launcher = textwrap.dedent(f"""\
        ...
        # Runs inside Terminal.app on Host B so the spawned listener
        # inherits Terminal.app's TCC permissions (Accessibility /
        # Screen Recording).
        set -eu
        ...
        # The binary is installed and invoked directly as {DEFAULT_REMOTE_BINARY_PATH};
        # its basename `ksh` is a control-shell name, so it serves MCP.
        # No symlink and no separate `tai` file — one file, one name.
        ...
        pkill -f '^{DEFAULT_REMOTE_BINARY_PATH} --listen' >/dev/null 2>&1 || true
        ...
        # this parent bash exits and the listener is reparented to launchd.
        nohup {DEFAULT_REMOTE_BINARY_PATH} --listen {port} --token '{token}' \\
...
# ---------------------------------------------------------------------------
# Step 3 — wait for the listener to bind. Check over SSH, not TCP,
...
def wait_for_listener(target: str) -> None:
    """Poll the listener log on the remote for the bind banner.
    ...
    The log banner ("tai: ksh listening on 127.0.0.1:PORT ...")
    ...
    """
    sys.stderr.write("mcp-bridge: waiting for the listener to bind\n")
    attempts = max(1, int(LISTENER_READY_TIMEOUT_S / 0.2))
    remote = (
        f"i=0; "
        f"while [ $i -lt {attempts} ]; do "
        f"  if grep -q 'ksh listening on' {DEFAULT_REMOTE_LOG_PATH} "
...
def probe_remote_session(target: str, port: int) -> tuple[str, str | None]:
    ...
    `/tmp/ksh --listen <port>` process, a readable token, and a bind
    ...
    r = _ssh(
        target,
        f"pgrep -f '^{DEFAULT_REMOTE_BINARY_PATH} --listen {port}' "
...
def count_remote_sessions(target: str, port: int) -> int:
    ...
    r = _ssh(
        target,
        f"pgrep -f '^{DEFAULT_REMOTE_BINARY_PATH} --listen {port}' | wc -l",
        timeout=20.0,
    )
    if r.returncode != 0:
        return -1
...
def cleanup_remote(target: str) -> None:
    """True agentless: remove the binary (/tmp/ksh), launcher, token,
    log, and the entire ~/Library/Caches/tai/ tree (extracted bundle
    payloads + bridge.log + template cache).
    ...
    per-session token, launcher, log, and cache tree are still wiped,
    but the next session finds the sha-matching binary already present
    and skips the (~191 MB) re-ship. Trades a resident binary on the
    remote for fast reconnects; default (unset) is the full agentless
    wipe."""
    ...
    # subdirs in one shot. When keep_binary is set, /tmp/ksh is left in
    ...
    cmd = (
        f"pkill -f '^{DEFAULT_REMOTE_BINARY_PATH} --listen' 2>/dev/null; "
...
def main() -> None:
    ...
    try:
        ...
        if mode == "attach":
            ...
            # session ships the binary; overwriting /tmp/ksh while
